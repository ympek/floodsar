...
readRasterDirectory(std::string dirname, std::string fileExtension, RasterInfoExtractor *extractor) {
    ...
    std::cout << "Scanning directory: " << dirname << " for " << fileExtension << " images..." << std::endl;
...
void reprojectIfNeeded(std::vector<RasterInfo> &rasters, std::string epsgCode) {
  ...
  std::cout << "Reprojection done. Reprojected " <<  reprojectedCount << " images, skipped " << skippedCount << " images\n";
}
...
int main(int argc, char **argv) {
    ...
    if (userInput.count("cache-only")) {
      ...
      if (!userInput.count("aoi")) {
        std::cout << "Area of interest not provided, use -o option. Program will quit\n";
        return 0;
      }
